...
```python
cont_df, sex_df, race_df = run_cohort_descriptives(biomarkers, disease, engine, filters, df=None)
```

### `run_biomarker_descriptives`
Descriptive statistics per biomarker with skewness flagging for log transformation.
```python
results_df = run_biomarker_descriptives(biomarkers, disease, engine, filters, df=None)
```

### `run_distribution_plots`
Raw and log-transformed distribution plots for each biomarker. Accepts a pre-filtered DataFrame.
```python
run_distribution_plots(biomarkers, disease, engine, filters, df=None, save_path=None)
```
...
results_df = run_correlation(biomarkers, disease, engine, method="spearman",
                             log_transform=False, filters=None, df=None)
```

### `run_scatter`
Scatter plot with regression line for a single biomarker vs outcome.
```python
run_scatter(biomarker, disease, engine, hue_col="sex_label", filters=None, save_path=None, df=None)
```
...
model = run_linear_regression(biomarkers, disease, engine,
                              log_transform=False, filters=None, covariates=None, df=None)
```
